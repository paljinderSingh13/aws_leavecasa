...

{
    ...
    public function  hotel_detail( $sid , $code){
       ...
       // dd($image);
        ...
        $hotel_res= $hotel_api->request('GET','https://api-sandbox.grnconnect.com/api/v3/hotels/availability/'.$sid.'?hcode='.$code,[
            'headers' => [
                'Content-Type' => 'application/json',
                'api-key' => 'b12092d579e8795a77c3abe759d6185f',
                'Accept' => 'application/json',
     ...
     public function hotel_book_status(Request $request)
    {
        ...
        $bref = $request->bref;
        $status = new Client();
        $status_res= $status->request('GET','https://api-sandbox.grnconnect.com/api/v3/hotels/bookings/'.$bref.'?type=GRN',[
            'headers' => [
                'Content-Type' => 'application/json',
                'api-key' => 'b12092d579e8795a77c3abe759d6185f',
                'Accept' => 'application/json',
    ...
    {
             $cancel = new Client();
        $cancel_res= $cancel->request('DELETE','https://api-sandbox.grnconnect.com/api/v3/hotels/bookings/'.$bref,[
            'headers' => [
                'Content-Type' => 'application/json',
                'api-key' => 'b12092d579e8795a77c3abe759d6185f',
                'Accept' => 'application/json',
...
protected function fare_quote($index){
    ...
    public function hotelsResults(Request $request){
        ...
        // dump( Session::get('hotel_req'), $hotel);
...
dd($booking_item, $nresults);
    ...
    protected function re_fetch_check($sid, $array){
        ...
        $nclient = new Client();
        $nres = $nclient->request('GET','https://api-sandbox.grnconnect.com/api/v3/hotels/availability/'.$sid.'?rates='.$array['rate_key'],[
            'headers' => [
                'Content-Type' => 'application/json',
                'api-key' => 'b12092d579e8795a77c3abe759d6185f',
                'Accept' => 'application/json',
    ...
    public function hotel_book(Request $request){
        ...
        if(!empty($request['rate_index'])){
            ...
            if(count($req_info['rooms']) != count($request['rate_index'])){
            
            // dd('heree');
            Session::flash('error','Number of room choose incorrect');
            return back();    
            }
            ...
            foreach ($req_info['rooms'] as $key => $value) {

                $key = $value['adults'];
                if(!empty($value['children_ages'])){
                    $key .= '_'.count($value['children_ages']);
                }
                
                $r_a_c[$key][] = 1; 
            }

            dd($r_a_c , $req_info['rooms']);
       ...
       // dd(Session::get('rates'), $request->all());
        ...
        $recheck = new Client();
        $re_res = $recheck->request('post','https://api-sandbox.grnconnect.com/api/v3/hotels/availability/'.$request->search_id.'/rates/?action=recheck',[
            'headers' => [
                'Content-Type' => 'application/json',
                'api-key' => 'b12092d579e8795a77c3abe759d6185f',
                'Accept' => 'application/json',
...
//dump($request->all());
    ...
    $client = new Client();
        $re_res = $client->request('post','https://api-sandbox.grnconnect.com/api/v3/hotels/bookings',[
            'headers' => [
                'Content-Type' => 'application/json',
                'api-key' => 'b12092d579e8795a77c3abe759d6185f',
                'Accept' => 'application/json',
