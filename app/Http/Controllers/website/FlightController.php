        ...
        if(session::has('markup')) {
            Session()->forget('markup');
               ...
               // dd($results);
                ...
                if(($tripType == 'one_way') && ($errorStatus==true || count($results['Response']['Results']) ==1 )) {
                    return view('frontend.pages.flight.search_flight.one_way',['results'=>$results,'action'=>$action,'airlines'=>$airlines,'tripType'=>$tripType,'errorStatus'=>$errorStatus,'errorMessage'=>$errorMessage]);
                }


                 if(($tripType == 'round') && ($errorStatus==true || count($results['Response']['Results']) ==2 )) {
                    return view('frontend.pages.flight.search_flight.round_way',['results'=>$results,'action'=>$action,'airlines'=>$airlines,'tripType'=>$tripType,'errorStatus'=>$errorStatus,'errorMessage'=>$errorMessage]);

                }
                if(($tripType == 'multi_city') && ($errorStatus==true || count($results['Response']['Results']) ==1 )) {
                    return view('frontend.pages.flight.search_flight.multi_way',['results'=>$results,'action'=>$action,'airlines'=>$airlines,'tripType'=>$tripType,'errorStatus'=>$errorStatus,'errorMessage'=>$errorMessage]);
 ...
 {$trips_data =1;}      
    ...
    $errorStatus = false;
            ...
            // dd($results);
                ...
                if(($request->trip_type == 'one_way') && ($errorStatus==true || count($results['Response']['Results']) ==1 )) {
                    return view('frontend.pages.flight.search_flight.one_way',['results'=>$results,'action'=>$action,'airlines'=>$airlines,'tripType'=>$request->trip_type,'errorStatus'=>$errorStatus,'errorMessage'=>$errorMessage]);
                }


                 if(($request->trip_type == 'round') && ($errorStatus==true || count($results['Response']['Results']) ==2 )) {
                    return view('frontend.pages.flight.search_flight.round_way',['results'=>$results,'action'=>$action,'airlines'=>$airlines,'tripType'=>$request->trip_type,'errorStatus'=>$errorStatus,'errorMessage'=>$errorMessage]);

                }
                if(($request->trip_type == 'multi_city') && ($errorStatus==true || count($results['Response']['Results']) ==1 )) {
                    return view('frontend.pages.flight.search_flight.multi_way',['results'=>$results,'action'=>$action,'airlines'=>$airlines,'tripType'=>$request->trip_type,'errorStatus'=>$errorStatus,'errorMessage'=>$errorMessage]);
...
// dd($requestData);
 ...
 // dump(3, $requestData);
        ...
        return view('frontend.pages.flight.flight_detail.one_detail',['request_data'=>$requestData]);//, 'fare_rule'=>$fare_rule]);
    }
